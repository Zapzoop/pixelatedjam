...
func _on_animation_frame_changed() -> void:
	...
	if state == SUMMON && sprite.frame == 3:
		...
		if rage < RAGE_SPAM_THRESH:
			...
			minion_count = 1 + randi() % 4
		else:
			spawn_cooldown = SPAWN_COOLDOWN * 0.75
...
func _spawn_aoe_attack(type: int) -> void:
	"""
	AOE Attack type
	0: expanding
	...
	match type:
		0:
			...
			var distance = 1.0
			var cycles = 5 if rage > RAGE_SPAM_THRESH else 3
			...
			for _cycle in range(cycles):
				for dir in dirs:
					for i in [-1.0, 1.0]:
						...
						aoe_attack.global_position = caster_pos + (dir * i) * 16.0 * distance
		...
		1:
			...
			for cycle in range(cycles):
				...
				aoe_attack.global_position = caster_pos + (target_dir + spawn_bias * rand_range(0.05, 0.25)) * 8.0 * cycle
				
				for _frame in range(24):
					yield(tree, "idle_frame")
		
		2:
			var cycles = 5 if rage > RAGE_SPAM_THRESH else 13
			...
			for cycle in range(cycles):
				...
				aoe_attack.global_position = playerDetectionZone.player.global_position
